...
### Step 1：建立工作區與複製影片
1. 決定內部影片代號，例如 `<video-id>`（如 `20260822-agent-intro`）。
...
2. 產生字幕純文字稿：
   ...
   ```
3. 建立輸出目錄：`output/<影片標題> [<Agent>]/`（`<Agent>` 為 Claude／Codex／OpenCode／Antigravity，依執行中的 agent 決定）。
4. 將成品複製更名至輸出目錄：
   - 影片：`output/<標題> [<Agent>]/<標題>.mp4`
   - 字幕：`output/<標題> [<Agent>]/<標題>.srt`
   - 文字：`output/<標題> [<Agent>]/<標題>.txt`
